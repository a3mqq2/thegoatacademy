          ...
          <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row mb-2">
              <div class="col-md-6">
                ...
                <div class="mb-2">
                  @if($user->avatar)
                    <img src="{{ asset('storage/'.$user->avatar) }}" alt="Avatar" class="rounded mb-2" style="max-height: 100px;">
                  @endif
                </div>
              ...
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="cost_per_hour" class="form-label">
                    <i class="fa fa-money-bill-wave"></i> Cost Per Hour:
                  </label>
                  <input type="number" name="cost_per_hour" id="cost_per_hour"
                         class="form-control @error('cost_per_hour') is-invalid @enderror"
                         value="{{ old('cost_per_hour', $user->cost_per_hour) }}"
                         placeholder="Enter cost per hour" required>
            ...
            <div id="instructorFields">
              ...
              <div class="row">
                ...
                <div class="col-md-12 mt-3">
                  <label for="levels" class="form-label">
                    <i class="fa fa-code"></i> Levels:
                  </label>
                  <select name="levels[]" id="levels" class="form-select @error('levels') is-invalid @enderror" multiple>
                    @foreach($levels as $level)
                      <option value="{{ $level->id }}"
                        {{ in_array($level->id, old('levels', $user->levels->pluck('id')->toArray())) ? 'selected' : '' }}>
                        {{ $level->name }}
                      </option>
                    @endforeach
                  </select>
              ...
              <div class="mb-3">
                ...
                <textarea name="notes" id="notes" rows="4"
                          ...
                          placeholder="Enter any additional notes">{{ old('notes', $user->notes) }}</textarea>
                      ...
                      <tr>
                        <td>
                          <select name="shifts[][day]" class="form-control" required>
                            ...
                            @endphp
                            @foreach($daysList as $dayOption)
                              <option value="{{ $dayOption }}" 
                                {{ $dayOption == $shift->day ? 'selected' : '' }}>
                                {{ $dayOption }}
                              </option>
                            @endforeach
                          </select>
                        ...
                        <td>
                          <input type="time" name="shifts[][start_time]" class="form-control"
                                 value="{{ $shift->start_time }}" required>
                        </td>
                        <td>
                          <input type="time" name="shifts[][end_time]" class="form-control"
...
@push('scripts')
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function () {
      ...
      // Show/hide permission groups based on checked roles
      ...
      FilePond.create(videoInputElement, {
        server: {
          process: {
            url: '{{ route("upload.file") }}', // Adjust to your route
            method: 'POST',
            headers: {
          ...
          revert: {
            url: '{{ route("upload.file.revert") }}', // Adjust to your route
            method: 'DELETE',
            headers: {
              'X-CSRF-TOKEN': '{{ csrf_token() }}'
            }
